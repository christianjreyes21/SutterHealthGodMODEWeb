...
filOutput1 = "pcSerial.txt"

If objFSO.FileExists(filOutput1) Then 
objFSO.DeleteFile(filOutput1) 
End If

Set objOutputFile1 = objFSO.OpenTextFile (filOutput1, ForAppending, True)
...
For each objBIOS in colBIOS
	objOutputFile1.WriteLine(objBIOS.SerialNumber) 
Next

filOutput = "pcPrinters.txt" 
...
filOutput2 = "pcMonitor.txt"


If objFSO.FileExists(filOutput2) Then 
objFSO.DeleteFile(filOutput2) 
End If
...
tmpOutput=tmpOutput & tmpResult(0) & vbcrlf
tmpOutput=tmpOutput & tmpResult(4) & vbcrlf
next
Set objOutputFile2 = objFSO.OpenTextFile (filOutput2, ForAppending, True)

objOutputFile2.WriteLine(tmpOutput)
...
'strComputer = Wscript.Arguments(0) 
 ...
 Set objWMIService = GetObject("winmgmts:{impersonationLevel=impersonate}!\\" & strComputer & "\root\cimv2") 
 'Set colInstalledPrinters = objWMIService.ExecQuery("Select Name from Win32_Printer Where Local = False") 
 Set colInstalledPrinters = objWMIService.ExecQuery("Select Name from Win32_PrinterConfiguration") 

 For Each objPrinter in colInstalledPrinters 
     printers = printers & objPrinter.Name & vbcrlf
 Next 
 'printers = colInstalledPrinters.ItemIndex(0).Name
